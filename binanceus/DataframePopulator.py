...
def add_indicators(dataframe: DataFrame) -> DataFrame:
    ...
    global startup_win, win_size, runmode
    global n_profit_stddevs, n_loss_stddevs
    ...
    if runmode in ('hyperopt', 'backtest', 'plot'):
        dataframe['dwt'] = dataframe['close'].rolling(window=startup_win).apply(roll_get_dwt)
        dataframe['smooth'] = dataframe['close'].rolling(window=startup_win).apply(roll_smooth)
        dataframe['dwt_smooth'] = dataframe['dwt'].rolling(window=startup_win).apply(roll_smooth)
    else:
        dataframe['dwt'] = get_dwt(dataframe['close'])
    ...
    dataframe['profit_threshold'] = dataframe['dwt_profit_mean'] + n_profit_stddevs * abs(dataframe['dwt_profit_std'])

    dataframe['loss_threshold'] = dataframe['dwt_loss_mean'] - n_loss_stddevs * abs(dataframe['dwt_loss_std'])
    ...
    DataframeUtils.check_inf(dataframe)
...
def add_hidden_indicators(dataframe: DataFrame) -> DataFrame:
    ...
    dataframe['dwt_low'] = dataframe['dwt_smooth'].rolling(window=startup_win).min()
    dataframe['dwt_high'] = dataframe['dwt_smooth'].rolling(window=startup_win).max()
...
def add_future_data(dataframe: DataFrame, lookahead: int) -> DataFrame:
    ...
    if use_dwt:
        ...
        future_df['full_dwt'] = get_dwt(dataframe['close'])
    ...
    future_df['future_profit_mean'] = future_df['future_profit'].rolling(win_size).mean()
    future_df['future_profit_std'] = future_df['future_profit'].rolling(win_size).std()
    future_df['future_profit_max'] = future_df['future_profit'].rolling(win_size).max()
    future_df['future_profit_min'] = future_df['future_profit'].rolling(win_size).min()
    future_df['future_loss_mean'] = future_df['future_loss'].rolling(win_size).mean()
    future_df['future_loss_std'] = future_df['future_loss'].rolling(win_size).std()
    future_df['future_loss_max'] = future_df['future_loss'].rolling(win_size).max()
    future_df['future_loss_min'] = future_df['future_loss'].rolling(win_size).min()

    # future_df['profit_threshold'] = future_df['profit_mean'] + n_profit_stddevs * abs(future_df['profit_std'])
    # future_df['loss_threshold'] = future_df['loss_mean'] - n_loss_stddevs * abs(future_df['loss_std'])

    future_df['future_profit_threshold'] = future_df['dwt_profit_mean'] + n_profit_stddevs * abs(
        future_df['dwt_profit_std'])
    future_df['future_loss_threshold'] = future_df['dwt_loss_mean'] - n_loss_stddevs * abs(future_df['dwt_loss_std'])
    ...
    future_df['future_nseq_up_thresh'] = future_df['future_nseq_up_mean'] + n_profit_stddevs * future_df[
        'future_nseq_up_std']
    ...
    future_df['future_nseq_dn_thresh'] = future_df['future_nseq_dn_mean'] \
                                         - n_loss_stddevs * future_df['future_nseq_dn_std']
...
def add_stoploss_indicators(dataframe) -> DataFrame:
...
def get_dwt(col):
    ...
    restored_sig = dwtModel(a_notrend)
