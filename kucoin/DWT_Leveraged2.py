...
This version does not use the 'base' pair for trends
"""
...
class DWT_Leveraged2(IStrategy):
    ...
    entry_short_dwt_diff = DecimalParameter(0.1, 5.0, decimals=1, default=-2.0, space='buy', load=True, optimize=True)
    ...
    exit_short_dwt_diff = DecimalParameter(-5.0, -0.1, decimals=1, default=2.0, space='sell', load=True, optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if (self.isBull(curr_pair)) or (self.isBear(curr_pair)):
            ...
            if (self.isBull(curr_pair)) or (self.isBear(curr_pair)):
                ...
                dataframe = merge_informative_pair(dataframe, informative, self.timeframe, self.inf_timeframe,
                                                   ffill=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.isBull(metadata['pair']):
            ...
            # Trend
            long_conditions.append(dataframe['candle-dn-trend'] == 1)
        ...
        # 'Bear'/short leveraged token
            ...
            # Trend
            short_conditions.append(dataframe['candle-dn-trend'] == 1)

            # DWT triggers
            short_dwt_cond = (
                    qtpylib.crossed_above(dataframe['dwt_model_diff'], self.entry_short_dwt_diff.value)
            )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 'Bear'/short leveraged token
            ...
            # DWT triggers
            short_dwt_cond = (
                qtpylib.crossed_below(dataframe['dwt_model_diff'], self.exit_short_dwt_diff.value)
            )
